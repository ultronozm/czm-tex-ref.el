...
The same copy/yank workflow extends to cross-document references in the style of the [[https://ctan.org/pkg/xr][xr]]/[[https://ctan.org/pkg/xr-hyper][xr-hyper]] packages: run =czm-tex-ref-label= in the document containing the target label, switch to the referencing document, and yank.  If the reference was copied from a different file, yanking it also inserts the corresponding =\externaldocument{...}= declaration into the preamble (after any existing declarations, after the =xr=/=xr-hyper= usepackage line, or before =\begin{document}=, adding =\usepackage{xr-hyper}= in the last case), unless it is already declared.  If an existing declaration specifies a prefix, that prefix is applied to the yanked reference.  Set =czm-tex-ref-manage-externaldocument= to =nil= to disable this.
